...
export class ProductsService {
  ...
  saveProduct(product: INewProductRequest): Observable<INewProductResponse> {
    return this._httpClient.post<INewProductResponse>(`${environment.apiUrl}/api/products`, product);
  }

  getProducts(): Observable<IProductsResponse> {
    return this._httpClient.get<IProductsResponse>(`${environment.apiUrl}/api/products`);
  }
}
